        ...
        for l in range(seqlen):
            ...
            for w in range(width):
                ...
                if input_l >= 0:
                    ...
                    var weight_val: Scalar[weight_dtype] = (
                        w0 if w == 0 else (w1 if w == 1 else (w2 if w == 2 else w3))
                    )
...
fn causal_conv1d_channel_first_fwd_gpu[
    ...
    if kWidth == 1:
        w_single = Scalar[x_dtype](weight.ptr.offset(weight_c_base + UInt32(0) * weight_width_stride).load())
    elif kWidth == 2:
        var w0_val = Scalar[x_dtype](weight.ptr.offset(weight_c_base + UInt32(0) * weight_width_stride).load())
        var w1_val = Scalar[x_dtype](weight.ptr.offset(weight_c_base + UInt32(1) * weight_width_stride).load())
        W_2 = SIMD[x_dtype, 2](w0_val, w1_val)
    elif kWidth == 4:
        var w0_val = Scalar[x_dtype](weight.ptr.offset(weight_c_base + UInt32(0) * weight_width_stride).load())
        var w1_val = Scalar[x_dtype](weight.ptr.offset(weight_c_base + UInt32(1) * weight_width_stride).load())
        var w2_val = Scalar[x_dtype](weight.ptr.offset(weight_c_base + UInt32(2) * weight_width_stride).load())
        var w3_val = Scalar[x_dtype](weight.ptr.offset(weight_c_base + UInt32(3) * weight_width_stride).load())
    ...
    if prev_chunk_col >= 0 and prev_chunk_col * kNElts < nSeqLen:
        ...
        for i in range(kNElts):
            ...
            if prev_seq_idx >= 0 and prev_seq_idx < nSeqLen:
                var prev_offset: UInt32 = UInt32(batch_id) * x_batch_stride + UInt32(channel_id) * x_c_stride + UInt32(prev_seq_idx) * x_l_stride
                prev_input_chunk[i] = Scalar[x_dtype](x.ptr.offset(prev_offset).load())
    ...
    if current_chunk_col >= 0 and current_chunk_col * kNElts < nSeqLen:
        ...
        for i in range(kNElts):
            ...
            if curr_seq_idx < nSeqLen:
                var curr_offset: UInt32 = UInt32(batch_id) * x_batch_stride + UInt32(channel_id) * x_c_stride + UInt32(curr_seq_idx) * x_l_stride
                input_chunk[i] = Scalar[x_dtype](x.ptr.offset(curr_offset).load())
    ...
    for i in range(kNElts):
        ...
        if kWidth == 1:
            if seq_idx >= 0 and seq_idx < nSeqLen:
                var load_offset: UInt32 = UInt32(batch_id) * x_batch_stride + UInt32(channel_id) * x_c_stride + UInt32(seq_idx) * x_l_stride
            ...
            for w in range(2):
                ...
                if input_l >= 0 and input_l < nSeqLen:
                    var load_offset: UInt32 = UInt32(batch_id) * x_batch_stride + UInt32(channel_id) * x_c_stride + UInt32(input_l) * x_l_stride
                    input_window[w] = Scalar[x_dtype](x.ptr.offset(load_offset).load())
            ...
            if input_l0 >= 0 and input_l0 < nSeqLen:
                var load_offset: UInt32 = UInt32(batch_id) * x_batch_stride + UInt32(channel_id) * x_c_stride + UInt32(input_l0) * x_l_stride
                x0 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
            if input_l1 >= 0 and input_l1 < nSeqLen:
                var load_offset: UInt32 = UInt32(batch_id) * x_batch_stride + UInt32(channel_id) * x_c_stride + UInt32(input_l1) * x_l_stride
                x1 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
            if input_l2 >= 0 and input_l2 < nSeqLen:
                var load_offset: UInt32 = UInt32(batch_id) * x_batch_stride + UInt32(channel_id) * x_c_stride + UInt32(input_l2) * x_l_stride
                x2 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
            conv_result = w0 * x0 + w1 * x1 + w2 * x2
        
        var out_val: Scalar[output_dtype] = Scalar[output_dtype](cur_bias) + Scalar[output_dtype](conv_result)
        if silu_active:
            out_val = out_val / (1 + exp(-out_val))
        out_vals[i] = out_val
    ...
    for i in range(kNElts):
        ...
        if seq_idx >= seq_end:
            break
        var out_offset: UInt32 = UInt32(batch_id) * out_batch_stride + UInt32(channel_id) * out_c_stride + UInt32(seq_idx) * out_l_stride
...
fn causal_conv1d_channel_last_fwd_gpu[
    ...
    for c_offset in range(kNElts):
        ...
        var W: SIMD[weight_dtype, kWidth] = rebind[type_of(W)](weight_v[c_idx, 0])
        ...
        if prev_chunk_col >= 0 and prev_chunk_col * kNElts < nSeqLen:
            ...
            if prev_row_2d >= 0 and prev_row_2d < nBatches * nSeqLen:
                ...
                if prev_batch >= 0 and prev_batch < nBatches and prev_seq >= 0 and prev_seq < nSeqLen:
                    ...
                    for i in range(kNElts):
                        ...
                        if c_idx_load < nChannels:
                            var prev_offset: UInt32 = prev_batch * x_batch_stride + prev_seq * x_l_stride + c_idx_load * x_c_stride
                            prev_input_chunk[i] = Scalar[x_dtype](x.ptr.offset(prev_offset).load())

        var current_chunk_col: Int = seq_start // kNElts
        var input_chunk: SIMD[x_dtype, kNElts] = 0
        if current_chunk_col >= 0 and current_chunk_col * kNElts < nSeqLen:
            var current_row_2d: Int = batch_id * nSeqLen + current_chunk_col * kNElts
            if current_row_2d >= 0 and current_row_2d < nBatches * nSeqLen:
                ...
                if current_batch >= 0 and current_batch < nBatches and current_seq >= 0 and current_seq < nSeqLen:
                    # Load SIMD vector manually using pointer arithmetic
                    input_chunk = 0
                    @parameter
                    for i in range(kNElts):
                        var c_idx_load: Int = channel_start + i
                        if c_idx_load < nChannels:
                            var current_offset: UInt32 = current_batch * x_batch_stride + current_seq * x_l_stride + c_idx_load * x_c_stride
                            input_chunk[i] = Scalar[x_dtype](x.ptr.offset(current_offset).load())
        
        var x_vals: SIMD[x_dtype, kNElts * 2] = prev_input_chunk.join(input_chunk)
        ...
        for i in range(kNElts):
            ...
            for w in range(kWidth):
                ...
                if input_l >= 0 and input_l < nSeqLen:
                    var load_offset: UInt32 = batch_id * x_batch_stride + input_l * x_l_stride + c_idx * x_c_stride
                    input_window[w] = Scalar[x_dtype](x.ptr.offset(load_offset).load())
            
            var tmp: SIMD[output_dtype, kWidth] = rebind[type_of(tmp)](input_window * rebind[type_of(input_window)](W))
        ...
        for i in range(kNElts):
            ...
            if seq_idx >= seq_end:
                break
            var out_offset: UInt32 = batch_id * out_batch_stride + seq_idx * out_l_stride + c_idx * out_c_stride
...
fn causal_conv1d_channel_last_fwd_gpu_with_seq_idx[
    ...
    for c_offset in range(kNElts):
        ...
        @parameter
        if kWidth >= 1:
            w0 = Scalar[weight_dtype](weight.ptr.offset(c_idx * weight_c_stride + 0 * weight_width_stride).load())
        @parameter
        if kWidth >= 2:
            w1 = Scalar[weight_dtype](weight.ptr.offset(c_idx * weight_c_stride + 1 * weight_width_stride).load())
        @parameter
        if kWidth >= 3:
            w2 = Scalar[weight_dtype](weight.ptr.offset(c_idx * weight_c_stride + 2 * weight_width_stride).load())
        @parameter
        if kWidth >= 4:
            w3 = Scalar[weight_dtype](weight.ptr.offset(c_idx * weight_c_stride + 3 * weight_width_stride).load())
        ...
        for i in range(kNElts):
            ...
            var cur_seq_idx_offset: UInt32 = batch_id * seq_idx_batch_stride + seq_pos * seq_idx_l_stride
            ...
            if kWidth == 1:
                ...
                if input_l >= 0 and input_l < nSeqLen:
                    var input_seq_idx_offset: UInt32 = batch_id * seq_idx_batch_stride + input_l * seq_idx_l_stride
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    var input_seq_idx: Int32 = Int32(input_seq_idx_val)
                    if input_seq_idx == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + input_l * x_l_stride + c_idx * x_c_stride
                        var x_val = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                        conv_sum += Scalar[output_dtype](Scalar[output_dtype](x_val) * Scalar[output_dtype](w0))
            elif kWidth == 2:
                var x0: Scalar[x_dtype] = 0
                var x1: Scalar[x_dtype] = 0
                ...
                if input_l0 >= 0 and input_l0 < nSeqLen:
                    var input_seq_idx_offset: UInt32 = batch_id * seq_idx_batch_stride + input_l0 * seq_idx_l_stride
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + input_l0 * x_l_stride + c_idx * x_c_stride
                        x0 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                if input_l1 >= 0 and input_l1 < nSeqLen:
                    var input_seq_idx_offset: UInt32 = batch_id * seq_idx_batch_stride + input_l1 * seq_idx_l_stride
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + input_l1 * x_l_stride + c_idx * x_c_stride
                        x1 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                conv_sum += Scalar[output_dtype](Scalar[output_dtype](w0) * Scalar[output_dtype](x0) + Scalar[output_dtype](w1) * Scalar[output_dtype](x1))
            elif kWidth == 3:
                var x0: Scalar[x_dtype] = 0
                var x1: Scalar[x_dtype] = 0
                var x2: Scalar[x_dtype] = 0
                ...
                if input_l1 >= 0 and input_l1 < nSeqLen:
                    ...
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        ...
                        x1 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                if input_l2 >= 0 and input_l2 < nSeqLen:
                    var input_seq_idx_offset: UInt32 = batch_id * seq_idx_batch_stride + input_l2 * seq_idx_l_stride
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + input_l2 * x_l_stride + c_idx * x_c_stride
                        x2 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                conv_sum += Scalar[output_dtype](Scalar[output_dtype](w0) * Scalar[output_dtype](x0) + Scalar[output_dtype](w1) * Scalar[output_dtype](x1) + Scalar[output_dtype](w2) * Scalar[output_dtype](x2))
            else:  # kWidth == 4
                var x0: Scalar[x_dtype] = 0
                var x1: Scalar[x_dtype] = 0
                var x2: Scalar[x_dtype] = 0
                ...
                if input_l3 >= 0 and input_l3 < nSeqLen:
                    var input_seq_idx_offset: UInt32 = batch_id * seq_idx_batch_stride + input_l3 * seq_idx_l_stride
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + input_l3 * x_l_stride + c_idx * x_c_stride
                        x3 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                conv_sum += Scalar[output_dtype](Scalar[output_dtype](w0) * Scalar[output_dtype](x0) + Scalar[output_dtype](w1) * Scalar[output_dtype](x1) + Scalar[output_dtype](w2) * Scalar[output_dtype](x2) + Scalar[output_dtype](w3) * Scalar[output_dtype](x3))
        ...
        for i in range(kNElts):
            ...
            if seq_pos >= seq_end:
                break
            var out_offset: UInt32 = batch_id * out_batch_stride + seq_pos * out_l_stride + c_idx * out_c_stride
            output.ptr.offset(out_offset).store(out_vals_channel[i])
        ...
        for i in range(kNElts):
            ...
            if kWidth == 1:
                ...
                if input_l >= 0 and input_l < nSeqLen:
                    ...
                    var input_seq_idx: Int32 = Int32(input_seq_idx_val)
                    if input_seq_idx == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + c_idx * x_c_stride + input_l * x_l_stride
                        var x_val = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                        conv_sum += Scalar[output_dtype](Scalar[output_dtype](x_val) * Scalar[output_dtype](w0))
            elif kWidth == 2:
                var x0: Scalar[x_dtype] = 0
                ...
                if input_l0 >= 0 and input_l0 < nSeqLen:
                    ...
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + c_idx * x_c_stride + input_l0 * x_l_stride
                        x0 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                if input_l1 >= 0 and input_l1 < nSeqLen:
                    var input_seq_idx_offset: UInt32 = batch_id * seq_idx_batch_stride + input_l1 * seq_idx_l_stride
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + c_idx * x_c_stride + input_l1 * x_l_stride
                        x1 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                conv_sum += Scalar[output_dtype](Scalar[output_dtype](w0) * Scalar[output_dtype](x0) + Scalar[output_dtype](w1) * Scalar[output_dtype](x1))
            elif kWidth == 3:
                var x0: Scalar[x_dtype] = 0
                ...
                if input_l2 >= 0 and input_l2 < nSeqLen:
                    ...
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + c_idx * x_c_stride + input_l2 * x_l_stride
                        x2 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                conv_sum += Scalar[output_dtype](Scalar[output_dtype](w0) * Scalar[output_dtype](x0) + Scalar[output_dtype](w1) * Scalar[output_dtype](x1) + Scalar[output_dtype](w2) * Scalar[output_dtype](x2))
            else:  # kWidth == 4
                var x0: Scalar[x_dtype] = 0
                ...
                if input_l3 >= 0 and input_l3 < nSeqLen:
                    ...
                    var input_seq_idx_val = seq_idx.ptr.offset(input_seq_idx_offset).load()
                    if Int32(input_seq_idx_val) == cur_seq_idx:
                        var load_offset: UInt32 = batch_id * x_batch_stride + c_idx * x_c_stride + input_l3 * x_l_stride
                        x3 = Scalar[x_dtype](x.ptr.offset(load_offset).load())
                conv_sum += Scalar[output_dtype](Scalar[output_dtype](w0) * Scalar[output_dtype](x0) + Scalar[output_dtype](w1) * Scalar[output_dtype](x1) + Scalar[output_dtype](w2) * Scalar[output_dtype](x2) + Scalar[output_dtype](w3) * Scalar[output_dtype](x3))
...
fn causal_conv1d_update_cpu[
    ...
    x: LayoutTensor[x_dtype, x_layout, MutAnyOrigin],  # Shape (B, C, L) or (B, C) when L=1
    mut conv_state: LayoutTensor[conv_state_dtype, conv_state_layout, MutAnyOrigin],  # Shape (B, C, S)
    weight: LayoutTensor[weight_dtype, weight_layout, MutAnyOrigin],  # Shape (C, W)
    mut output: LayoutTensor[output_dtype, output_layout, MutAnyOrigin],  # Shape (B, C, L)
    bias: LayoutTensor[bias_dtype, bias_layout, MutAnyOrigin],  # Shape (C,)
        ...
        for c in range(dim):
            ...
            for l in range(seqlen):
                ...
                for w in range(width):
                    ...
                    if src_pos >= state_len:
                        ...
                        var x_offset = Int(b * x_batch_stride + c * x_c_stride + x_l_pos * x_l_stride)
                        input_val = x.ptr.offset(x_offset).load()
                    elif src_pos >= 0:
                        # Read from conv_state
                        var conv_state_offset = Int(b * conv_state_batch_stride + c * conv_state_c_stride + src_pos * conv_state_l_stride)
                        input_val = Scalar[x_dtype](conv_state.ptr.offset(conv_state_offset).load())
                ...
                # Write output
                var out_offset = Int(b * out_batch_stride + c * out_c_stride + l * out_l_stride)
                var out_val: Scalar[output_dtype] = conv_sum
                if silu_activation:
                    out_val = out_val / (1 + exp(-out_val))
                output.ptr.offset(out_offset).store(out_val)
            ...
            if seqlen >= state_len:
                ...
                for s in range(state_len):
                    ...
                    var conv_state_offset = Int(b * conv_state_batch_stride + c * conv_state_c_stride + s * conv_state_l_stride)
                    conv_state.ptr.offset(conv_state_offset).store(Scalar[conv_state_dtype](x_val))
            else:
                # Shift conv_state left by seqlen positions, then append x
                for s in range(state_len - seqlen):
                    var src_offset = Int(b * conv_state_batch_stride + c * conv_state_c_stride + (s + seqlen) * conv_state_l_stride)
                    var dst_offset = Int(b * conv_state_batch_stride + c * conv_state_c_stride + s * conv_state_l_stride)
                    var val = conv_state.ptr.offset(src_offset).load()
                    conv_state.ptr.offset(dst_offset).store(val)
                
                # Copy x values to the end
                for l in range(seqlen):
                    var x_offset = Int(b * x_batch_stride + c * x_c_stride + l * x_l_stride)
                    var x_val = x.ptr.offset(x_offset).load()
                    var conv_state_offset = Int(b * conv_state_batch_stride + c * conv_state_c_stride + (state_len - seqlen + l) * conv_state_l_stride)
                    conv_state.ptr.offset(conv_state_offset).store(Scalar[conv_state_dtype](x_val))
...
fn causal_conv1d_update_cpu_no_bias[
    ...
    mut conv_state: LayoutTensor[conv_state_dtype, conv_state_layout, MutAnyOrigin],
