...
# Optimized softplus - matches vLLM/Triton reference implementation
...
fn varlen_selective_state_update_gpu[
    ...
    stride_state_dim: UInt32,
    stride_state_dstate: UInt32,
    # Strides for x: (batch, nheads, dim)
    stride_x_batch: UInt32,
    stride_x_head: UInt32,
    stride_x_dim: UInt32,
    # Strides for dt: (batch, nheads, dim)
    stride_dt_batch: UInt32,
    stride_dt_head: UInt32,
    stride_dt_dim: UInt32,
    # Strides for dt_bias: (nheads, dim)
    stride_dt_bias_head: UInt32,
    stride_dt_bias_dim: UInt32,
    # Strides for A: (nheads, dim, dstate)
    stride_A_head: UInt32,
    stride_A_dim: UInt32,
    stride_A_dstate: UInt32,
    # Strides for B: (batch, ngroups, dstate)
    stride_B_batch: UInt32,
    stride_B_group: UInt32,
    stride_B_dstate: UInt32,
    # Strides for C: (batch, ngroups, dstate)
    stride_C_batch: UInt32,
    stride_C_group: UInt32,
    stride_C_dstate: UInt32,
    # Strides for D: (nheads, dim)
    stride_D_head: UInt32,
    stride_D_dim: UInt32,
    # Strides for z: (batch, nheads, dim)
    stride_z_batch: UInt32,
    stride_z_head: UInt32,
    stride_z_dim: UInt32,
    # Strides for out: (batch, nheads, dim)
    stride_out_batch: UInt32,
    stride_out_head: UInt32,
    ...
    if Bool(Int(has_state_batch_indices) != 0):
        state_batch_idx = state_batch_indices.ptr.offset(pid_b_int).load()
        # Check for padding
...
fn varlen_selective_scan_fwd_gpu[
    ...
    stride_delta_dim: UInt32,
    stride_delta_len: UInt32,
    ...
    stride_delta_bias_dim: UInt32,
    # Strides for ssm_states: (batch, dim, dstate)
    stride_ssm_batch: UInt32,
    stride_ssm_dim: UInt32,
    stride_ssm_dstate: UInt32,
