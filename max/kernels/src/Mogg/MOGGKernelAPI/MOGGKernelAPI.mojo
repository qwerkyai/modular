...
from nn.causal_conv1d import (
    ...
    naive_causal_conv1d_channel_first_fwd_cpu_no_bias,
    optimized_causal_conv1d_channel_first_fwd_cpu,
    optimized_causal_conv1d_channel_first_fwd_cpu_no_bias,
    naive_causal_conv1d_channel_first_fwd_gpu,
    naive_causal_conv1d_channel_first_fwd_gpu_no_bias,
...
struct Or(ElementwiseBinaryOp):
    ...
    fn elementwise[
        ...
        __comptime_assert (
            dtype == DType.bool
        ), "expected bool operands for mo.oor"
        return lhs | rhs
...
struct Matmul:
    ...
    fn execute[
        ...
        __comptime_assert not (packed_b and transpose_b), (
    ...
    fn execute[
        ...
        with Trace[TraceLevel.OP, target=target](
            ...
            if is_cpu[target]():
                ...
                __comptime_assert (input.rank == 4 and filter.rank == 4) or (
...
struct Struct_mla_prefill_ragged_plan:
    ...
    fn execute[
        ...
        __comptime_assert kv_blocks.static_spec.shape.get[1]() == 1, (
...
struct Struct_mla_decode_graph_paged:
    ...
    fn execute[
        ...
        with Trace[TraceLevel.OP, target=target](
            "mo.mla.graph.decode.paged", task_id=get_safe_task_id(context)
        ):
            mla_decode_branch_fp8[
                m_scale_granularity=m_scale_granularity,
                n_scale_granularity=n_scale_granularity,
...
struct Struct_grouped_matmul_ragged:
    ...
    fn execute[
        ...
        __comptime_assert is_gpu[target](), "grouped matmul only support GPUs"
...
struct Struct_grouped_matmul_dynamic_scaled_fp8:
    ...
    fn execute[
        ...
        __comptime_assert is_gpu[target](), (
...
fn layout_transform_conv_transpose_filter_common[
    ...
    __comptime_assert filter_rank + 1 == packed_filter_rank
...
fn layout_transform_conv_filter_common[
    ...
    __comptime_assert packed_rank == filter_rank + 1
...
struct DistributedMatmulAllReduce:
    ...
    fn execute[
        ...
        __comptime_assert not n_dim.is_dynamic(), "n dimension should be static"
        __comptime_assert not k_dim.is_dynamic(), "k dimension should be static"
...
struct QuantizeStaticScaledFloat8[*, scale_is_inverted: Bool]:
    ...
    fn execute[
        ...
        __comptime_assert is_gpu[target](), "only valid on GPUs"
        __comptime_assert output_type in (
            DType.float8_e4m3fn,
...
struct Struct_lora_sgmv_ragged:
    ...
    fn execute[
        ...
        __comptime_assert is_gpu[target](), "SGMV only supported on GPUs"
        cuda_ctx = context.get_device_context()
...
struct SpatialMerge:
    ...
    fn execute[
        ...
        __comptime_assert is_gpu[
            target
        ](), "spatial_merge only supported on GPUs"
...
struct CausalConv1D[algorithm: StaticString, activation: StaticString]:
    ...
    activation and multiple algorithm implementations.
    
    Parameters:
        algorithm: Kernel algorithm to use.
    ...
    Tensor Shapes:
        ...
        - xx2D: Scratch tensor for internal use (same shape as output).
    """
    @staticmethod
    fn execute[
        dtype: DType,
        ...
        output: OutputTensor[dtype=dtype, rank=rank],
        xx2D: OutputTensor[dtype=dtype, rank=2],
        input: FusedInputTensor[dtype=dtype, rank=rank],
        weight: InputTensor[dtype=dtype, rank=2],
        bias: InputTensor[dtype=dtype, rank=1],
        ctx: DeviceContextPtr,
        ...
        var xx2Do = xx2D.to_layout_tensor()
        ...
        if is_cpu[target]():
            ...
            if algorithm == "naive":
                ...
                raise Error("Unsupported algorithm for CPU")
        elif is_gpu[target]():
            ...
            if algorithm == "naive":
                ...
                gpu_ctx.enqueue_function_checked(
                    ...
                    grid_dim=(batch_size, ceildiv(dim, BX), ceildiv(seqlen, BX)),
                    block_dim=(BX),
                )
            elif algorithm == "optimized":
                ...
                if width == 1:
                    comptime kWidth = 1
                    var compiled_func_opt = gpu_ctx.compile_function_checked[
                        causal_conv1d_channel_first_fwd_gpu[
                            X.dtype,
                            X.layout,
                            W.dtype,
                    ...
                    comptime BX = 32
                    var compiled_func_fallback = gpu_ctx.compile_function_checked[
                        naive_causal_conv1d_channel_first_fwd_gpu[
                            X.dtype,
                            X.layout,
                            W.dtype,
                ...
                raise Error("Unsupported algorithm for GPU")
        else:
...
fn _varlen_selective_state_update_gpu(
    ...
    gpu_ctx.enqueue_function_checked(
        ...
        # State strides
        UInt32(state_strides[0]),
        UInt32(state_strides[1]),
        UInt32(state_strides[2]),
        UInt32(state_strides[3]),
        # x strides
        UInt32(x_strides[0]),
        UInt32(x_strides[1]),
        UInt32(x_strides[2]),
        ...
        UInt32(dt_strides[2]),
        # dt_bias strides
        UInt32(dt_bias_strides[0]) if dt_bias.dim_size(0) > 0 else UInt32(0),
        UInt32(dt_bias_strides[1]) if dt_bias.dim_size(0) > 0 else UInt32(0),
        # A strides
        UInt32(A_strides[0]),
        UInt32(A_strides[1]),
        UInt32(A_strides[2]),
        ...
        UInt32(D_strides[1]) if D.dim_size(0) > 0 else UInt32(0),
        # z strides
        UInt32(z_strides[0]) if z.dim_size(0) > 0 else UInt32(0),
        UInt32(z_strides[1]) if z.dim_size(0) > 0 else UInt32(0),
        UInt32(z_strides[2]) if z.dim_size(0) > 0 else UInt32(0),
        # out strides
        UInt32(out_strides[0]),
        UInt32(out_strides[1]),
        UInt32(out_strides[2]),
        grid_dim=(dim_blocks, batch, nheads),
        block_dim=(1,),  # Each block processes BLOCK_SIZE_M dims
    )
...
struct VarlenSelectiveStateUpdate:
    ...
    fn shape(
        ...
        pad_slot_id: InputTensor,
    ) raises -> IndexList[3]:
        # Output shape is (batch, nheads, dim)
        return IndexList[3](x.dim_size(0), x.dim_size(1), x.dim_size(2))
...
fn _varlen_selective_scan_fwd_gpu(
    ...
    var out_lt = delta_lt  # Output goes to delta or z
    ...
    gpu_ctx.enqueue_function_checked(
        ...
        UInt32(delta_strides[0]),  # out strides = delta strides
        UInt32(delta_strides[1]),
...
struct VarlenSelectiveScanFwd:
    ...
    fn shape(
        ...
        # Output shape for ssm_states: (batch, dim, dstate)
        # batch = query_start_loc.dim_size(0) - 1 (since query_start_loc is (batch + 1,))
        # dim = u.dim_size(0) (since u is (dim, total_length))
        # dstate = A.dim_size(1) (since A is (dim, dstate))
        var batch = query_start_loc.dim_size(0) - 1
        var dim = u.dim_size(0)
        var dstate = A.dim_size(1)
        return IndexList[3](batch, dim, dstate)
